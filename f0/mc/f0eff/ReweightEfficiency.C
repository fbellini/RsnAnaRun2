...
int ReweightEfficiency(/*TH1D* M,TF1* F,TH1D* G,TH1D* R,*/TString fileName = "reweighted.root",int test=1,int save=2,double tolerance=0.001){
...
.     3.) The histograms stored in the array c and named "[name of M]_correction_iX" are correction factors: the ratio of M(iteration X) to M(iteration 0).  Multiplying M(iteration 0) by this correction factor gives the final version of M, adjusted for the re-weighted efficiency.  This can be useful if you store the separate uncertainties of M (statistical and systematic) in different TH1 or TGraph objects.  Alternatively, you can calculate the reweighted efficiency from the final iterations of G and R (see note 5).
  ...
  TFile * file = new TFile(fileName.Data(), "RECREATE");
    ...
    TFile* f1 = TFile::Open("finalWsys_new.root");//open the file that contains your measured histogram
    if (!f1) return 1;
    //TH1D* M=(TH1D*) f1->Get("hCorrectedSpectrum_syst_stat");//get your measured histogram, the systematic uncertainties should be the sum in quadrature of the statistical and systematic uncertainties (but you may want to exclude sources of systematic uncertainty that are correlated between pT bins)
    TH1D* M = (TH1D*)f1->Get("hCorrectedSpectrum_syst_stat");
    if (!M) return 2;
    TFile* f3 = TFile::Open("levyFit_9010221.root");
    if (!f3) return 3;
    TF1* F = (TF1*)f3->Get("levy");
    if (!F) return 4;
    ...
    TFile* f2 = TFile::Open("real_AccxEff_2sTPC_3sTOFveto.root");//open the file that contains your simulated histograms
    if (!f2) return 5;
    TH1D* G=(TH1D*) f2->Get("hPtGeneratedF0");//get your generated histogram
    if (!G) return 6;
    TH1D* R=(TH1D*) f2->Get("hPtReconstructedF0");//get your reconstructed histogram
    if (!R) return 7;
    
    //TFile* f4=new TFile("output_file_reweight.root","RECREATE","HistoFile");//open the new file that will contain your output
    /*//gROOT->LoadMacro("*path/PWGLF/RESONANCES/macros/utils/ReweightEfficiency.C");
    ReweightEfficiency(M,F,G,R,f4,1);//run the macro
...
*/
  ...
  F->SetName(Form("%s_i0",fname.Data()));

  for(i=0;i<imax;i++){//iterate
    g[i][0]=(TH1D*) G->Clone(Form("%s_i%i",gname.Data(),i));
    g[i][1]=(TH1D*) M->Clone(Form("%s_rebin_i%i",gname.Data(),i));
    r[i][0]=(TH1D*) R->Clone(Form("%s_i%i",rname.Data(),i));
    r[i][1]=(TH1D*) M->Clone(Form("%s_rebin_i%i",rname.Data(),i));
    c[i]=(TH1D*) M->Clone(Form("%s_correction_i%i",mname.Data(),i));
    ...
    m[i]=(TH1D*) M->Clone(Form("%s_i%i",mname.Data(),i));
    ...
    //new fit
    F->SetName(Form("%s_i%i",fname.Data(),i));
    do_fit(m[i],F,test);
    if(save==2){
      file->cd();
      F->Write();
