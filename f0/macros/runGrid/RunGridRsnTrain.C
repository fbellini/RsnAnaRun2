...
void RunGridRsnTrain(TString pluginmode="test", 
		     ...
		     TString aliPhysicsVer = "vAN-20170819-1") { 
				    ...
				    TString aliPhysicsVer = "vAN-20160312-1")
{
  ...
  plugin->SetAdditionalLibs("libEventMixing.so libPWGLFresonances.so");
...
void SetupIO(TString suffix="_Example", Bool_t isMC = 0)
{
  ...
  if (isMC) {
    ...
    if (year>=2013) myGridDataDir.Append(Form("/%i",year));
    myGridDataDir.Append(Form("/%s",sim.Data()));
  }
  else myGridDataDir.Append(Form("/data/%i/%s",year, data.Data()));
