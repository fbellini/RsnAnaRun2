...
#include "ConfigF0V2.C"
//#include "AliRsnMiniAnalysisTaskV2.h"
#endif

AliRsnMiniAnalysisTaskV2 * AddTaskF0V2
(
...
{  
  ...
  */
  ...
  TString taskName = Form("RsnTaskF0V2");
  AliRsnMiniAnalysisTaskV2 *task = new AliRsnMiniAnalysisTaskV2(taskName.Data(), isMC);
  ...
  //-----------------------------------------------------------------------------------------------
  if (collSys==AliPIDResponse::kPP) task->SetMultiplicityEstimator("AliMultSelection_V0M");
  if (collSys==AliPIDResponse::kPPB) task->SetMultiplicityEstimator("AliMultSelection_V0A");
  if (collSys==AliPIDResponse::kPBPB) task->SetMultiplicityEstimator("AliMultSelection_V0M");
  ...
  Printf("Event mixing configuration: \n events to mix = %i \n max diff. vtxZ = cm %5.3f \n max diff multi = %5.3f", nmix, maxDiffVzMix, maxDiffMultMix);
  ...
  //-----------------------------------------------------------------------------------------------
   ...
   cutVertex->SetCheckZResolutionSPD(); Printf("CheckZResolutionSPD:              ON");
  cutVertex->SetCheckDispersionSPD(); Printf("CheckDispersionSPD:               ON");
  cutVertex->SetCheckZDifferenceSPDTrack(); Printf("CheckZDifferenceSPDTrack:         ON");
  ...
  Printf("CheckIncompleteDAQ:                  ON");
  Printf("SetCheckSPDClusterVsTrackletBG:      ON");
  ...
  if (useMVPileUpSelection){
    ...
    Printf("Multiple-vtx Pile-up rejection:      ON \nSettings: MinPlpContribMV = %i, MinPlpContribSPD = %i", MinPlpContribMV, MinPlpContribSPD);
  } else {
    cutEventUtils->SetMinPlpContribSPD(MinPlpContribSPD);
    Printf("SPD Pile-up rejection:                      ON \nSettings: MinPlpContribSPD = %i", MinPlpContribSPD);
  }
  Printf("Pile-up rejection mode:                %s", (rejectPileUp?"ON":"OFF"));   
  ...
  // -- CONFIG ANALYSIS --------------------------------------------------------------------------
  //-----------------------------------------------------------------------------------------------
  if (!ConfigF0V2(task, isMC, collSys, cutsPair, enaMultSel, masslow, massup, nbins, aodFilterBit, cutPiPid, nsigma, enableMonitor) ) return 0x0;
