		     ...
		     TString username, TString aliPhysicsVer)
{
  ...
  TString myOutDir  = Form("phiXeXe_%s", (isMC? sim.Data() : data.Data()));
  ...
  if(useAlienPlugin) {
    ...
    //Order matters!!!!
    plugin->SetAdditionalLibs("liblhapdf.so libEGPythia6.so libpythia6.so libAliPythia6.so AddTaskPhiXeXe.C ConfigPhiXeXe.C");
