void PostProcessQAPhi(char* name_fin="LHC17d1_B0p2.root",char* name_fout="QAphi_LHC17d1_B0p2",char* name_list="RsnQA_phi",char* name_hist_base="taskRsnQA_phi_",Bool_t* MC=kTRUE, 
		      int lowptbin=3, int highptbin=4){
  ...
  hs_plot2=(TH1D*) hs->Clone("mass_signal_plot2");//temporary histogram for plotting
  ...
  hu->SetTitle("Bkgd-subtracted");
  hu->SetXTitle("#it{M}_{KK} (GeV/#it{c}^{2})");
  hu->SetYTitle("Counts / (1 MeV/#it{c}^{2})");
  hu->Draw();
  hl->Draw("same");
  ...
  hs->SetTitle("Background Subtracted");
  hs->SetXTitle("#it{M}_{KK} (GeV/#it{c}^{2})");
  hs->SetYTitle("Counts / (1 MeV/#it{c}^{2})");
  hs->Draw();
  ...
  //----- plot the fits -----
  TCanvas * cfit2 = new TCanvas("cfit2","cfit2",10, 10, 800, 800);
  cfit2->SetFillColor(0);
  cfit2->cd();
  hs_plot2->GetXaxis()->SetRangeUser(0.995,1.07-1e-5);
  hs_plot2->SetLineColor(1);
  hs_plot2->SetMarkerColor(1);
  hs_plot2->SetMarkerStyle(20);
  hs_plot2->SetTitle(hs->GetTitle());
  hs_plot2->GetYaxis()->SetTitleSize(0.05);
  hs_plot2->GetYaxis()->SetTitleOffset(1.20);
  hs_plot2->GetXaxis()->SetTitleSize(0.05);
  hs_plot2->SetXTitle(hs->GetXaxis()->GetTitle());
  hs_plot2->SetYTitle("Counts / (1 MeV/#it{c}^{2})");
  gb[0][2]->SetLineColor(kBlue);
  gb[0][2]->SetLineWidth(3);
  gb[0][2]->SetLineStyle(7);
  g[0][2]->SetLineColor(kRed);
  g[0][2]->SetLineWidth(3);
  hs_plot2->Draw();
  gb[0][2]->Draw("same");
  g[0][2]->Draw("same");

  gStyle->SetOptTitle(0);
  gROOT->LoadMacro("~/alice/macro/AddPaveText.C");
  AddPaveTextXeXe("tr", 0.04, "");
  
  TLegend * leg = new TLegend(0.6, 0.47, 0.85, 0.65, Form("%2.1f < #it{p}_{T} < %2.1f GeV/#it{c}", lowpt, highpt));
  leg->SetBorderSize(0);
  leg->SetFillStyle(0);
  leg->SetTextSize(0.03);
  leg->AddEntry(hs_plot2, "Data (LSB subtracted)", "pl");
  leg->AddEntry(g[0][2], "Signal peak fit (Voigtian)", "l");
  leg->AddEntry(gb[0][2], "Residual Bg. fit", "l");
  leg->Draw();  
  return;
  
  TCanvas * cfit = new TCanvas("cfit","cfit",10,10,1000,750);
  cfit->SetFillColor(0);
  ...
  cfit->SaveAs(Form("%s.pdf)",name_fout));
