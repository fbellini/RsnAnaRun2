...
void Ratio2Kaons(Bool_t plotThermal = 0)
{
  ...
  leg->AddEntry(gPhi2KaPbPb, "Pb-Pb #sqrt{#it{s}_{NN}} = 5.02 TeV Prelim.", "p");
  ...
  TFile * fout = new TFile("PhiToKaonRatio.root","recreate");
  fout->cd();
...
void GetRatio_XeXe(Double_t &x, Double_t &y, Int_t phiCent, TH1F * kaStat, TGraphErrors * phiStat)
{
 ...
 if (phiCent == 0){
    ...
    kaYmerged    = (kaStat->GetBinContent(5) + kaStat->GetBinContent(6)) / 2.0;
    kaYmergedErr = (kaStat->GetBinError(5) + kaStat->GetBinError(6)) / 2.0;
  } else if (phiCent == 3) {
    kaYmerged    = (kaStat->GetBinContent(7) + kaStat->GetBinContent(8)) / 2.0;
    kaYmergedErr = (kaStat->GetBinError(7) + kaStat->GetBinError(8)) / 2.0;
  } else if (phiCent == 4) {
    kaYmerged    = kaStat->GetBinContent(9);
    kaYmergedErr = kaStat->GetBinError(9);
  }
...
{
  TString KaonXeFileName = "/Users/fbellini/alice/resonances/RsnAnaRun2/phiXeXe/final/piKp/YieldAndMeanPtSumKwSys_2020.root";
  //"~/alice/resonances/RsnAnaRun2/phiXeXe/preliminaryQM18/Preliminary_XeXe_YieldAndMeanPtSumKwSys.root";  
  ...
  TGraphErrors * gPhi2KaXeXe = new TGraphErrors(5);
  TGraphErrors * gPhi2KaXeXe_syst = new TGraphErrors(5);
...
{
  TString KaonPbFileName = "~/alice/resonances/RsnAnaRun2/phiXeXe/preliminaryQM18/Preliminary_YieldAndMeanPtSumKawSys_PbPb5TeV.root";  
