...
void Ratio2Pions(Bool_t plotThermal = 0)
{
  ...
  TFile * fout = new TFile("Preliminary_PhiToPionRatio.root","recreate");
  fout->cd();
...
void GetRatio_XeXe(Double_t &x, Double_t &y, Int_t phiCent, TH1F * piStat, TGraphErrors * phiStat)
{
  ...
  if (phiCent == 0){
    ...
    piYmerged    = (piStat->GetBinContent(8)*0.5 + piStat->GetBinContent(9)) / 1.5;
    piYmergedErr = (piStat->GetBinError(8)*0.5 + piStat->GetBinError(9)) / 1.5;
  }
...
{
  TString PionXeFileName = "Preliminary_XeXe_YieldAndMeanPtSumPiwSys.root";  
