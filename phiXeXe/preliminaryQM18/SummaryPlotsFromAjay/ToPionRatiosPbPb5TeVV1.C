...
void DefinePresence(){
  ...
  //ad-hoc selection
    ...
    presence [lphi]  [lppb] [lsyst] 	= kTRUE;
    //presence [lphi]  [lppb] [lsyst_unc] = kTRUE;
    
    
    presence [lk0s] [lpbpb] [lsyst] 	= kTRUE;
    presence [llam] [lpbpb] [lsyst] 	= kTRUE;
    presence [lphi] [lpbpb] [lsyst] 	= kTRUE;
    presence [lxi]  [lpbpb] [lsyst] 	= kTRUE;
    presence [lom]  [lpbpb] [lsyst] 	= kTRUE;
    
    
    //13 TeV points
    //presence [lp]   [lpp13] [lsyst_unc] 	= kTRUE;
    //presence [lp]   [lpp13] [lstat] 	= kTRUE;
    //presence [lk]   [lpp13] [lsyst] 	= kTRUE;
    //presence [lk]   [lpp13] [lstat] 	= kTRUE;
    
    presence [lphi] [lpp13] [lsyst] 	= kTRUE;
    //presence [lphi] [lpp13] [lsyst_unc] = kTRUE;
    presence [lphi] [lpp13] [lstat] 	= kTRUE;

    presence [lk0s] [lpp13] [lsyst] = kTRUE;
    //presence [lk0s] [lpp13] [lsyst_unc] = kTRUE;
    presence [lk0s] [lpp13] [lstat] 	= kTRUE;

    presence [llam] [lpp13] [lsyst] = kTRUE;
    //presence [llam] [lpp13] [lsyst_unc] = kTRUE;
    presence [llam] [lpp13] [lstat] 	= kTRUE;

    presence [lxi]  [lpp13] [lsyst] 	= kTRUE;
    //presence [lxi]  [lpp13] [lsyst_unc] = kTRUE;
    presence [lxi]  [lpp13] [lstat] 	= kTRUE;

    presence [lom]  [lpp13] [lsyst] 	= kTRUE;
    //presence [lom]  [lpp13] [lsyst_unc] = kTRUE;
    presence [lom]  [lpp13] [lstat] 	= kTRUE;
  
    
}

void ToPionRatiosPbPb5TeVV1(int plotmode=1 , bool linearlinear=kTRUE){
  ...
  //building graphs
    ...
    TFile *fin = new TFile("MakeRootFile/IdentifiedParticlesRatioPlots_AllPart.root","read");
    TFile *fin_2 = new TFile("MakeRootFile/IdentifiedParticlesRatioPlotsWithPhi_AA5.root","read");
    fin->cd();
...
//     }
  ...
  //formatting graphs
    if(plotmode!=6 && plotmode !=11){
      float gralpha[n_systems] = {0.5, 1., 1.,0.8};
      //			                     lpi, lk, lp,   lk0s,    llam, lphi,      lxi,    lom
      int grcol[n_systems][n_particles] =    { { 0,  0,  0, kBlack,   kBlue,    0, kGreen+2, kRed+1},  //pp
                                             { 0,  0,  0, kBlack,   kBlue,    kMagenta, kGreen+2, kRed+1},  //pPb
					       { 0,  0,  0, kBlack,   kBlue,   kMagenta, kGreen+2, kRed+1},//PbPb
					       { 0,  0,  0, kBlack,   kBlue,    kMagenta, kGreen+2, kRed+1}//pp 13
					   };
      int grmarker[n_systems][n_particles] = { { 0,  0,  0, 24,     24,   0,    24,      24},  //pp
					       { 0,  0,  0, 27,     27,  27,    27,      27},  //pPb
					       { 0,  0,  0, 25,     25,   25,    25,      25},   //PbPb
					       { 0,  0,  0, 20,     20,   20,    20,      20}  //pp 13
					   };
      float grmarsiz[n_systems][n_particles] = { { 0., 0., 0., 1.5.,  1.5,  0.,  1.5,  1.5},  //pp
						 { 0., 0., 0., 2., 2., 2., 2., 2.},  //pPb
						 { 0., 0., 0., 1.5,  1.5,  1.5, 1.5, 1.5},   //PbPb
						 { 0., 0., 0., 1.5,  1.5,  1.5, 1.5,  1.5}   //pp 13
					   };
    }
    else{
	...
	for(int i_htype=0; i_htype<3; i_htype++){
	  ...
	  if(i_htype==lsyst_unc) {
	    ...
	    gr[i_part][i_system][i_htype]->SetFillColorAlpha(grcol[i_system][i_part],gralpha[i_system]);
	  }
	}
      }
    }
...
//       gr_PHEN[lom][lsyst]->SetFillColor(grcol[lpbpb][lom]+1);
  ...
  // for drawop==6 we change x values for PbPb  
    if(plotmode==6){
      ...
      TFile *fin_L = new TFile("~/Work_HeavyFiles/StrVsMult/pwglf-ppvsmult/QM2015/StrVsMult/macros/v0s/cLambda2Pi.root","read");
    ...
    //scale
    if(plotmode==1 || plotmode==6 || plotmode==11) {float scalefactor[n_particles] = {0,0,0,1,2,5,4,16};}
    //if(plotmode==1 || plotmode==6 || plotmode==11) {float scalefactor[n_particles] = {0,0,0,1,1,10.,4,10};}
    ...
    for(int i_part=lk0s; i_part<n_particles; i_part++){
      if(presence[i_part][lpp13][lsyst]) ScaleGraph(i_part,scalefactor[i_part]);
    }
  ...
  //frame
    if(plotmode==1 || plotmode==11) {
      double xrangeframe[2] = {2.,2500};
      //double yrangeframe[2] = {0.9e-3,1.9e-1};
      double yrangeframe[2] = {0.9e-3,2.e-1};
    }
    else if(plotmode==2) {
...
//       gr_PHEN[llam][lsyst]->Draw("P");
  ...
  //     PlotMC("K0S","Pythia6-Perugia2011-WithCR","Pythia6-P2011-WithCR",grcol[lpp][lk0s],scalefactor[lk0s],9);
      //PlotMC("K0S","Pythia8-Monash-WithCR","PYTHIA8",grcol[lpp][lk0s],scalefactor[lk0s],1);
      // PlotMC("K0S","DIPSY-ColorRopes","DIPSY ",grcol[lpp][lk0s],scalefactor[lk0s],2);
      // PlotMC("K0S","EPOSLHC","EPOS LHC",grcol[lpp][lk0s],scalefactor[lk0s],3);
...
//    if(plotmode==1 || plotmode==11) can->SaveAs("img_ToPionRatios_iterative.pdf");
    if(plotmode==1 || plotmode==11) can->SaveAs("HadronToPionRatiospp13TeVPbPb5TeVV1.png");
    if(plotmode==1 || plotmode==11) can->SaveAs("HadronToPionRatiospp13TeVPbPb5TeVV1.pdf");
    if(plotmode==1 || plotmode==11) can->Print("HadronToPionRatiospp13TeVPbPb5TeVV1.eps");
...
//     TLegend *leg = new TLegend(0.22,0.78,0.57,0.96,NULL,"brNDC");
    ...
    TLegend *leg = new TLegend(0.45,0.15,0.85,0.15+0.25,NULL,"brNDC");
    leg->SetBorderSize(0);
...
//     leg->AddEntry(gr[lom][lpbpb][lsyst],"#scale[0.9]{#splitline{ALICE Pb-Pb #sqrt{#it{s}_{NN}} = 2.76 TeV}{centrality classes}}","p");
    //leg->AddEntry(gr[lk0s][lpp][lstat]  ,"#scale[1]{pp, #sqrt{#it{s}} = 7 TeV}, Nat. Phys 13 (2017) 535","p");
    ...
    leg->AddEntry((TObject *)NULL, "#scale[1]{ALICE Preliminary}", "");
    leg->AddEntry(gr[lk0s][lpbpb][lsyst],"#scale[1]{Pb-Pb, #sqrt{#it{s}_{NN}} = 5.02 TeV}","p");
    leg->AddEntry(gr[lk0s][lpp13][lstat]  ,"#scale[1]{pp, #sqrt{#it{s}} = 13 TeV}","p");
...
//    leg_th = new TLegend(0.53,0.11,0.88,0.21,NULL,"brNDC");
   //if(plotmode==1) leg_th = new TLegend(0.42,0.10,0.71,0.25,NULL,"brNDC");
   ...
   if(plotmode!=6) leg_th->SetTextSize(leg_textsize_MC*0.95);
   else leg_th->SetTextSize(leg_textsize_MC*1.15);
...
void DrawParticles(float *scalef, int plotmode=1){
  ...
  if(plotmode==1 || plotmode==11){
    ...
    //lat.DrawLatex(500,0.0085,Form("#color[2]{#Omega^{#minus}+#bar{#Omega}^{#plus}} (#times%d)",(int)scalef[lom]));
    lat.DrawLatex(500,0.0085,Form("#Omega^{#minus}+#bar{#Omega}^{#plus} (#times%d)",(int)scalef[lom]));
    lat.DrawLatex(500,1.*y_lat[lxi],Form("#Xi^{#minus}+#bar{#Xi}^{#plus} (#times%d)",(int)scalef[lxi]));
    lat.DrawLatex(500,0.069,Form("#Lambda+#bar{#Lambda} (#times%d)",(int)scalef[llam]));
    lat.DrawLatex(500,0.042,Form("#phi (#times%d)",(int)scalef[lphi]));
    ...
    lat.DrawLatex(50,y_lat[lk0s]*0.8,"2K^{0}_{S}");
  }
  if(plotmode==3){
    for(int i=lk0s+1; i<n_particles; i++){
      if(presence[i][lpbpb][lsyst]) gr[i][lppb][lsyst]->GetPoint(/*gr[i][lppb][lsyst]->GetN()-1*/0,dum,y_lat[i]);
